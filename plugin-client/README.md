...
| `fortune_qimen` | **洛书九宫盘 SVG**：地盘干/九星/八门/八神四层（逐层开关）、值符宫呼吸描边、伏吟/反吟徽章；**多流派并算切换**：三元定局派（拆补法/茅山法）与值使起法派（时支本位宫/自旬首宫顺逆数地支）一键切换重排全盘 | 四层开关按钮；悬停宫位显示明细；流派按钮组 |
| `fortune_qizheng` | **黄道盘 + 双视图**：左盘平面发光描边（宿度/宫位/四余◆虚点/盘外读数标签）；右图日心轨道（视位黄经准线、月相、黄道带±5°、行星明度）/地心黄道天球双模式，可俯仰缩放转台相机、屏幕空间标签、命宫金弧、120/144Hz 自适应 | 悬停/点击锁定联动；拖拽视角；▶运转/⏺定格 + 进度滑块；紫气口径对照 |
...
- **大六壬天地盘**：平面双环带（地盘外环带 30° 分段 + 浮起天盘内盘）；三传金线沿环带
  描画；悬停/点击扇区 → 该支的天盘神/天将/遁干/六亲 浮层（可钉选）；
- **七政星盘（双联面板）**：左侧星盘（宫环带 + 宿度刻线 + 行星引线，标签按角度
  自动**内外圈交替避让**且**永不旋转**）；右侧**太阳系模型**——太阳居中、七政轨道
  按真实比例椭圆排布、行星为球体（渐变+高光）、**月球绕地球**、四余为虚线虚轨
  （罗计孛气），星盘与模型**双向联动高亮**（悬停任一视图星体两边同步亮起）；
  运转为 **JS 驱动的真实恒星周期模拟**（月 27.3 日→土 29.46 年→紫气 28 年映射
  30–104 秒），可暂停/播放；紫气六口径点位点击切换；
- **奇门九宫**：宫名居中（坎一宫…中五（寄坤二）），中列布局：宫名/地盘干（三奇
  乙丙丁金色）/九星/八门（吉凶配色）；值符（金）/值使（蓝）角标分置左右不再打架；
  **顺逆布灵动**沿宫序循环扫过（阳遁顺、阴遁逆），四层开关保留。
