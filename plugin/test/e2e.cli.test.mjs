...
test("e2e：全部工具 × 全部广告参数 真跑 CLI（契约）", { timeout: 300000, skip: SKIP }, async () => {
  ...
  r = await t.fortune_bazi.execute({
    ...BIRTH, lng: 112.454, trueSolar: true, dayChange: 23,
  ...
  r = await t.fortune_ziwei.execute({
    ...BIRTH, lng: 112.454, trueSolar: true, dayChange: 23, dst: false,
  ...
  // 7) 小六壬
  r = await t.fortune_xiaoliuren.execute({ month: 1, day: 18, hourZhi: "未" });
