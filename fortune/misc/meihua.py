...
争议说明：
- 动爻所在卦为「用」、不动为「体」是通行主流；个别流派以静卦为用，
  本项目采用主流约定。
- 《梅花易数》题宋·邵雍撰，学界多认为系后人托名；本模块与 CLI 引注均如实标注。
- 体用生克断语与《梅花易数》体用总诀原文一致（「体克用，诸事吉；用克体，诸事凶。
  体生用，有耗失之患；用生体，有进益之喜。体用比和，则百事顺遂」），
  原文存档 research/fetched/meihua_yishu.txt（通行排印本·网络转载版）。
"""
