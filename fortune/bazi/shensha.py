...
# 《渊海子平》论羊刃「甲丙戊庚壬五阳有刃，乙己丁辛癸五阴无刃」）
YANGREN = {"甲": "卯", "丙": "午", "戊": "午", "庚": "酉", "壬": "子"}
#: 少数派阴刃（帝旺位），默认不用。《渊海子平》足本另有一篇「论羊刃」
#: 列阴干禄前一位刃（乙辰、丁未、己未、辛戌、癸丑），亦为别传，均不采。
...
# 魁罡（《渊海子平》论魁罡：「壬辰、庚戌、戊戌、庚辰日是也」；
# 《诗诀》「壬戌」疑为刊误，从正文四日）
KUI_GANG = {"庚辰", "庚戌", "壬辰", "戊戌"}

# 日贵（《渊海子平》论日贵：「止有四日：丁酉、丁亥、癸巳、癸卯耳」）
RI_GUI = {"丁酉", "丁亥", "癸巳", "癸卯"}

# 金神（《渊海子平》论金神：「止有三时：癸酉、己巳、乙丑」）
JIN_SHEN = {"癸酉", "己巳", "乙丑"}
...
def compute(chart: BaziChart, base: str = "day") -> list[ShenShaHit]:
    ...
    gan_hit("天乙贵人", TIANYI, key_gan,
            note="版本一（主流，甲戊庚牛羊，《三命通会》卷三）；"
                 "《渊海子平》足本作「庚辛逢马虎」属版本二（庚→寅午），两古本各持一说，本仓从版本一")
    ...
    gan_hit("学堂", XUETANG, key_gan,
            note="日干长生位；《三命通会》卷三论学堂词馆「长生乃学堂之正位」。"
                 "《渊海子平》足本「论十干学堂」按纳音生人取（金巳、木亥、水土申、火寅），"
                 "本仓采日干派并如实标注两说")
    ...
    if dgz in SHI_E:
        hits.append(ShenShaHit("十恶大败", f"日柱{dgz}", ["日柱"], [dgz],
                               note="十干禄入空亡；《三命通会》卷三论十恶大败。"
                                    "《三命通会》维基文库本与《渊海子平》足本均作「乙丑」，"
                                    "按「禄入空亡」穷举恰为十日、应为「己丑」，两古本或同源之误，从通行"))

    # 魁罡 / 日贵 / 金神（《渊海子平》论魁罡 / 论日贵 / 论金神）
    if dgz in KUI_GANG:
        hits.append(ShenShaHit("魁罡", f"日柱{dgz}", ["日柱"], [dgz],
                               note="《渊海子平》论魁罡「壬辰、庚戌、戊戌、庚辰日是也」"
                                    "（《诗诀》「壬戌」疑为刊误，从正文四日）"))
    if dgz in RI_GUI:
        hits.append(ShenShaHit("日贵", f"日柱{dgz}", ["日柱"], [dgz],
                               note="《渊海子平》论日贵「止有四日：丁酉、丁亥、癸巳、癸卯耳」"
                                    "（原文兼论昼夜贵人之分）"))
    tgz = chart.pillar("时柱").gan_zhi
    if tgz in JIN_SHEN:
        hits.append(ShenShaHit("金神", f"时柱{tgz}", ["时柱"], [tgz],
                               note="《渊海子平》论金神「止有三时：癸酉、己巳、乙丑」"
                                    "「金神乃破败之神，要制伏，入火乡为胜」"))
