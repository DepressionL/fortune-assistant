...
def test_bazi_out_of_range():
    r = _invoke("bazi", "-y", "999", "-m", "2", "-d", "24", "-H", "13")
    assert r.exit_code == 2
...
def test_liuyao_random_ok():
    r = _invoke("liuyao", "--month-zhi", "申", "--day-ganzhi", "乙亥", "--random")
...
def test_xiaoliuren_invalid_hour_zhi():
    r = _invoke("xiaoliuren", "--month", "1", "--day", "18", "--hour-zhi", "猫")
    assert r.exit_code == 2
    assert "时支" in r.output


def test_chenggu_female_warns():
    r = _invoke("chenggu", "-y", "1997", "-m", "2", "-d", "24", "-H", "13", "-g", "女")
    assert r.exit_code == 0
