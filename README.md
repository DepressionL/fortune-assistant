...
1. **钟面时间（记录值）**：默认按**北京时间（UTC+8）**理解。海外出生按当地标准时
   ...
   先扣 1 小时。
2. **地方平太阳时**：`4×(经度−120)` 分钟。太原（112.454°E）即 −30.2 分钟——这是
   「经度差」项。
3. **真太阳时**：再加**均时差 EoT**（视太阳时−平太阳时，随日期变化，2 月中约
   −14 分、11 月初约 +16 分）。报告分项显示，如太原 1997-02-24：
   「经度差 −30.2 分 + 均时差 −13.3 分 = −43.5 分」，13:05 → 12:21:31（午时）。
...
- 《周易》卦辞/爻辞（`fortune/misc/zhouyi.py`）：与一份独立默写结果程序化逐字
  比对（去句读/爻题，`tests/fixtures/` + `tests/diff_zhouyi.py`），64 卦辞 + 384
  爻辞仅 4 处差异且全部为已知经典异文（咷/啕×2、己/巳×2，已入模块 YIWEN 表，
  本表从阮刻注疏本）；比对已固化为回归测试。**局限**：本环境网络被阻断，未能
  在线抓取权威数字版全文比对，此处为「独立记忆互证」而非版本级校勘，特此说明。
- 十干四化（紫微）：由 x_iztro 引擎盘面直接提取（716,314 条黄金用例的独立实现），
  并与通行十干四化表核对一致（脚本核验）。
- 六爻断语规则（`fortune/liuyao/duanyu.py`）：进神/退神 8 对、六合/六冲、六神
  主事经独立默写互证一致；六亲持世通论在《增删卜易》《卜筮正宗》传本间文字互有
  出入，模块只取各本一致的通义并已在注释与断语中如实标注。
